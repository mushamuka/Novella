...
//echo 'idarticle'.$_GET['produit']/*.$_GET['vendeur'].$_GET['qtetotale'].$_GET['qtevendue'].$_GET['qteretour'].$_GET['montant'].$_GET['datevente'].$_GET['frais_divers'].$_GET['note']*/.'qte'.$_GET['quantite'].'idpro=>'.$_GET['idproduction'];
	...
	//echo "voici ".$qteretour.' : prix produit :'.$_GET['prix']; 

//if ($_GET['produit'] == 1) 
//{
	$manquant = ($_GET['qtevendue']*$_GET['prix'])-($_GET['frais_divers']+$_GET['montant']);
	//echo $manquant;
	if ($vente->creevente($_GET['produit'],$_GET['vendeur'],$_GET['qtetotale'],$_GET['qtevendue'],$qteretour,$_GET['montant'],$manquant,$_GET['datevente'],$_GET['frais_divers'],$_GET['note'],$_GET['dette']) >0) 
	{
