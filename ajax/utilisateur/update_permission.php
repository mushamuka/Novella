...
if ($utilisateur->update_permission($_GET['idpermission'],$_GET['lire'],$_GET['creer'],$_GET['modifier'],$_GET['supprimer'],$_GET['page_accept'])) 
{
	//require_once('rep.php');
	$i = 0;
  	foreach ($utilisateur->affiche_permission_par_profil($_GET['profil_id']) as $value) 
	{ 
	$i++;
	?>
	<tr>
	  ...
	  <div class="modal-dialog modal-lgs">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	        <span id="msg<?=$i?>"></span>
	        <button type="button" class="btn btn-success" onclick="update_permission($('#idpermission<?=$i?>').val(),$('#lire<?=$i?>').val(),$('#creer<?=$i?>').val(),$('#modifier<?=$i?>').val(),$('#supprimer<?=$i?>').val(),$('#i<?=$i?>').val())" data-dismiss="modal">Modifier
	        </button>
